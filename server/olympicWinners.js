...
class OlympicWinners {
    ...
    buildSql(request) {
        ...
        const fromSql = '  FROM call.historico AS h LEFT JOIN call.detalles AS d ON (d.idhistorico = h.id) INNER JOIN call.usuarios AS u ON (u.id = h.idusuario) INNER JOIN call.gruposusuarios AS g ON (u.idgrupo = g.id) ';
...
export default new OlympicWinners();